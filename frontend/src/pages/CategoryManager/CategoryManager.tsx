...
const CategoryManager = () => {
  ...
  const filteredCategories = categories.filter((category) => category.name.toLowerCase().includes(searchedCategory.toLowerCase()));

  if (loading) return <p className="text-center mt-10">Chargement...</p>
  if (error) return <p className="text-center text-red-500 mt-10">Erreur de chargement des catégories</p>;
  ...
  const handleUpdateCategory = async (evt: React.FormEvent) => {
    ...
    const form = evt.target as HTMLFormElement;
    const formData = new FormData(form);
    const formJson = Object.fromEntries(formData.entries()) as UpdateCategoryInput;
  ...
  return (
    <div className="p-6 space-y-8 min-h-[80vh]">
      {/* Recherche & Compteur */}
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-6">
        ...
        <div className="w-full sm:w-auto">
          ...
          <div className="flex items-center bg-white rounded-full border-2 border-[#706EEB] px-4 py-2 shadow-md">
            ...
            />
            <span className="material-symbols-outlined text-[#706EEB] cursor-pointer ml-2"
        ...
        <div className="bg-white border-2 border-[#706EEB] px-6 py-2 rounded-xl shadow-md text-center">
          <p className="text-[#706EEB] text-sm font-medium">Nombre de catégories</p>
          <p className="text-3xl font-bold text-[#706EEB]">{categories.length}</p>
        </div>
      </div>

      {/* Bouton ajouter */}
      <div>
        <button className="bg-[#706EEB] text-white px-4 py-2 rounded-full shadow-md hover:bg-[#5a58d6] cursor-pointer transition"
        ...
        <div className="fixed inset-0 bg-black/20 my-0 flex items-center justify-center z-50 transition-opacity duration-300 ease-in-out animate-fade-in">
          <div className="relative bg-white max-w-md w-full px-6 py-5 rounded-xl shadow-lg border-2 border-[#706eeb] transform transition-all duration-300 ease-out scale-95 animate-scale-in">
            ...
            <div className="text-sm text-[#706eeb] font-medium space-y-2 mt-6">
              ...
              >
                <label htmlFor="name" className="block text-sm/6 font-medium text-gray-900 mb-1">Catégorie</label>
                ...
                <label htmlFor="color" className="block text-sm/6 font-medium text-gray-900 mb-1">Couleur</label>
                <div className="border border-gray-300 rounded-[25px] px-3 py-2">
                  <input
                    className="w-full outline-none text-gray-800 bg-transparent pl-2"
                    name="color"
                ...
                <label htmlFor="description" className="block text-sm/6 font-medium text-gray-900 mb-1">Déscription</label>
                <textarea
                  name="description"
                  rows={4}
          ...
          <div className="relative bg-white max-w-md w-full px-6 py-5 rounded-xl shadow-lg border-2 border-[#706eeb] transform transition-all duration-300 ease-out scale-95 animate-scale-in">
            ...
            <div className="text-sm text-[#706eeb] font-medium space-y-2 mt-6">
              ...
              >
                ...
                <label htmlFor="description" className="block text-sm/6 font-medium text-gray-900 mb-1">Description</label>
                <textarea
                  name="description"
                  rows={4}
                  className="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"
                  defaultValue={selectedCategoryData.getCategoryById.description ?? ""}>
