...
export default function EditInterestPointForm({
	interestPoint,
	onClose,
	isOpen,
}: EditInterestPointFormProps) {
	const { loading, error, data } = useGetCategoriesQuery();
	const [
		replaceInterestPoint,
		{ data: editedData, loading: submitting, error: editError },
	] = useReplaceInterestPointByIdMutation();
	...
	const { fetchInterestPointsByCity, fetchInterestPointById } =
				...
				<form onSubmit={handleSubmit}>
					...
					<div className="details-content space-y-4 text-sm sm:text-base">
						...
						<button
							type="button"
							onClick={onClose}
							className="flex justify-center items-center w-full border border-gray-300 text-sm px-4 py-2 rounded-md  hover:cursor-pointer hover:text-gray-500"
